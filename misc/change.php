<?php
    ...
    if ($_POST) {
        ...
        if ($_POST["type"] == "password") {
            if ($result = $mysqli->query("SELECT password FROM accounts WHERE id=" . intval($_SESSION["userdatas"]["id"]))) {
        ...
        if ($_POST["type"] == "address") {
            if ($result = $mysqli->query("UPDATE accounts SET address='" . $_POST["new"] . "' WHERE id=" . intval($_SESSION["userdatas"]["id"]))) {
                if ($account = $mysqli->query("SELECT * FROM accounts WHERE id=" . intval($_SESSION["userdatas"]["id"]))) {
